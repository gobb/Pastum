...
use Pastum\Component\PAGI\Exception\MockedException;

/**
 * An AGI client implementation useful for mocking and testing ivr apps.
 */
class MockedClient extends AbstractClient
{
    /**
     ...
     * @var array
     */
    private $mockedResultStrings = array();
    ...
    public function onGetVariable($success, $value = null)
    {
        if ($success) {
    ...
    public function onGetFullVariable($success, $value = null)
    {
        if ($success) {
            $this->addMockedResult('200 result=1 (' . $value . ')');
        } else {
    ...
    {
        $this->nodes[$name] = new \Pastum\Component\PAGI\Node\MockedNode();
